 ...
 dispatch(handleOpenEngin(false))
  ...
  return (
    <div  className=' w-full h-[auto] '>
      ...
      <div className={` w-[full]   h-[400px] ${userdata?.backgrounpicture ? '' : 'bg-gray-200 '}   overflow-hidden  rounded-[7px]`} >
        <div className='w-full h-[100%] bg-cover bg-center' style={{ backgroundImage:`url(${bgimage})`}} >
        </div>     
      ...
      {(popupState) => (
        <React.Fragment>
          ...
          <Menu {...bindMenu(popupState)}>
            <MenuItem id='fontbt2' onClick={popupState.close}>Dowload <i className="fa-solid fa-download ml-[14px] text-sky-500 text-[20px] hover:text-sky-600 "></i></MenuItem>
            <MenuItem id='fontbt2' onClick={popupState.close}>Upload <i className="fa-solid fa-cloud-arrow-up ml-[20px] text-[orange] text-[20px] hover:text-[#ffa600ed] "></i></MenuItem>
            <MenuItem id='fontbt2' onClick={popupState.close}>Delete <i className="fa-solid fa-trash ml-[24px] text-[#ff003c] text-[20px] hover:text-[#ff004c] "></i></MenuItem>
          </Menu>
        </React.Fragment>
      )}
    </PopupState>
      ...
      <div className='ml-[230px] hidden  2lg:flex  flex-row  items-center justify-between'>
        <div className=''>
        <Link to='/BodyRoute'> 
        <span className=' text-[25px] hover:underline '>{userdata?.firstName} {userdata?.lastName}</span>
        </Link>
