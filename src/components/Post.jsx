...
const Post = ({
  ...
  return (
    <div key={data._id} className={`w-full h-[auto] ${daynight ? 'bg-[#070707]' : 'bg-slate-200' } transition-all duration-300    rounded mt-[10px]`}>
      ...
      {(popupState) => (
        <React.Fragment>     
         ...
         <Menu {...bindMenu(popupState)}>
            ...
            <MenuItem onClick={popupState.close} id='fontbt' ><span id={data?._id}  onClick={(e) => handleUpdate(e , data.description , data.picturePath  )} className='text-[15px] flex flex-row items-center'>Update <i className="fa-solid fa-cloud-arrow-up ml-[20px] text-sky-500 text-[20px] hover:text-[#004cff] "></i></span></MenuItem>
            <MenuItem onClick={popupState.close} id='fontbt'><span id={data?._id} onClick={(e) => handledelete(e)}  className='text-[15px] flex flex-row items-center'>Delete <i className="fa-solid fa-trash ml-[27px] text-[#ff003c] text-[20px] hover:text-[#e80348] "></i></span></MenuItem>
            </div>: 
            <div>
             ...
             <MenuItem onClick={popupState.close} id='fontbt' ><span  id={data?._id}  className='text-[15px] flex flex-row items-center '>Report <i className="fa-solid fa-triangle-exclamation ml-[20px] text-end text-orange-500 text-[20px] hover:text-orange-600 "></i></span></MenuItem>
    ...
    </PopupState>                  
                           {
                            ...
                            :  userId === data.userId ?
                               ''
                             :<i id={data.userId} onClick={(e) => handleAddFriends(e)} className="fa-solid fa-user-plus text-[20px]  cursor-pointer hover:text-cyan-400" ></i>
                           }    
        ...
        <React.Fragment >     
          ...
          <Menu {...bindMenu(popupState)} >
            ...
            <div>
            <MenuItem onClick={popupState.close}><span id='fontbt' onClick={() => handleCOmmentUpdate(comment.commentid , data?._id , comment.comment) } 
            ...
            <MenuItem onClick={popupState.close} id='fontbt2' ><span  id={data?._id} onClick={(e) => handleCommentDelete(e , comment.commentid )}  className='text-[13px] flex flex-row items-center '>Delete <i className="fa-solid fa-trash ml-[21px] text-end  text-[#ff003c] text-[20px] hover:text-[#e80348]"></i></span></MenuItem>
            </div>: 
            <div>
             ...
             <MenuItem onClick={popupState.close} id='fontbt2' ><span  id={data?._id}  className='text-[13px] flex flex-row items-center '>Report <i className="fa-solid fa-triangle-exclamation ml-[20px] text-end text-orange-500 text-[20px] hover:text-orange-600 "></i></span></MenuItem>
                  ...
                  <div className={` absolute top-0 w-full h-[40px] flex flex-row items-center justify-center ${ loding ? '' : 'hidden'}`}>
                  <DotPulse 
                  size={40}
                  speed={1.3} 
                  color="#FFA726" 
               ...
               <div className={`w-[93%] h-[40px] flex flex-row items-center justify-between  overflow-hidden  ${daynight ? 'bg-black'  : 'bg-white' } rounded-full m-auto `}>               
                 ...
                 <i id='instgram' className="fa-solid fa-gift text-[22px] xs:text-[27px] mx-[5px]  cursor-pointer"></i>               
                 ...
                 <div className='w-[70%] h-full flex flex-row items-center '>
                    ...
                    />
                    {onupdate ?
                    <FaRegCommentDots id={data?._id} onClick={saveUpdate} className='text-[25px] text-sky-500 mr-[10px] hover:text-sky-600 cursor-pointer ml-[10px]'/>
                  : 
                  <FaRegCommentDots id={data?._id} onClick={(e) =>handleComment(e)} className='text-[25px] text-sky-500 mr-[10px] hover:text-sky-600 cursor-pointer ml-[10px]'/>
                  }
